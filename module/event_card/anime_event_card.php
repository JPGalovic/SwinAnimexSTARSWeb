			...
			if($get_anime_event_card_ok)
			{
				if(!$get_anime_event_card_data->num_rows == 0) // Same as above what to do when no data is found
				{
					...
					echo('<section class="quater" id="event_card">');
						//Event Image
						echo('<img src="image/anime/'.$anime_event_row['ANIME_TITLE'].' '.$anime_event_row['SESSION_TYPE_ID'].$anime_event_row['SESSION_NUMBER'].'.jpg" alt="'.$anime_event_row['ANIME_TITLE'].'">');
