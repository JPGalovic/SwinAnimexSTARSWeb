			...
			if($get_anime_episode_ok)
			{
				...
				while($episode_row = $get_anime_episode_data->fetch_assoc())
				{
					echo('<section class="quater">');
						echo('<img src="image/anime/'.remove_illegal_char(strtolower($anime_event_row['ANIME_TITLE'])).'/episode/'.$episode_row['EPISODE_NUMBER'].'.jpg" alt="'.$anime_event_row['ANIME_TITLE'].' Episode '.$episode_row['EPISODE_NUMBER'].'">');
